...
/// `vault_smoke_config_case_<cases - 1>` (mirroring `harness_core_macros::fuzz_runner`'s
/// per-case fan-out), any other mode emits a single `vault_smoke_config`. The annotated fn's own
/// body is dropped; only its name and asyncness matter. The runtime bridge (not this macro) is
/// the source of truth for the config's contents — see `crates/macros/src/config_runner.rs` and
...
/// (and the macro itself) `use`d into scope, the same convention `harness_core_macros::fuzz_runner`
/// and `#[cross_vm_contract]` use.
#[proc_macro_attribute]
