//! The config-driven CLI (spec section 8): the [`Cli`] builder, the `run` / `validate` / `list`
//! subcommands, `{PREFIX}_*` env-var precedence folding, profile/suite selection, and the CI
//! exit-code contract. Generic over a [`CliDomain`]: the domain fixes the config extension, the
//! setup-request type, the extra CLI flags, and the env-var prefix, so this module stays free of
//! any one domain's specifics.
//!
//! [`Cli`] wraps a [`Registry`]: a user binary calls
//! [`Cli::new`]/[`Cli::env_file`]/[`Cli::register`] to build one up, then `.main().await` to parse
//! `std::env::args()`, dispatch, and return a [`std::process::ExitCode`]. `replay <artifact>` is
//! sugar for `run <artifact> --profile replay` (spec section 10): both dispatch through the same
//! `dispatch_run`/`run_with_config` path, so an artifact's `.toml`/`.json` extension, the registry,
//! and the exit-code contract are all unchanged. `--json-report` (spec section 9) accumulates every
//! selected profile's [`ErasedReport`] and writes the envelope once, after the whole invocation
//! finishes, in the private `run_selected` helper, which also writes a replay artifact (spec
//! section 10) for any fuzz/invariant/endurance profile that failed.
//!
//! Precedence (spec section 8), highest first: CLI flag, `{PREFIX}_*` env var, profile key,
//! `[defaults]`, built-in default. The last two tiers are already merged by the loader and applied
//! by [`resolve_profile`] — this module only ever builds the CLI+env layer ([`RunOptions`]) and
//! hands it down; it never re-implements the profile/defaults merge.
//!
//! Exit codes (the CI contract): `0` all runs passed, `1` at least one run failed with
//! `Bug`/`Invariant`, `2` failed with `Infra`/`Setup`/`Serialize`/`Export` only, `3` a config or
//! usage error. A suite or multi-profile invocation reports the worst code across every profile it
//! ran; the private [`combine`] helper (unit-tested below) is the one place that ordering is
//! decided.
...
/// The config-driven CLI builder, generic over a [`CliDomain`]. Wraps a [`Registry`] with the
/// `.env` path a user binary wants loaded, then drives `run` / `validate` / `list` via
/// [`Cli::main`].
///
/// ```no_run
/// # async fn demo() -> std::process::ExitCode {
/// use harness_cli::{Cli, GenericDomain};
/// Cli::<GenericDomain>::new()
///     .env_file(".env")
///     .main()
///     .await
/// # }
/// ```
pub struct Cli<D: CliDomain> {
...
impl<D: CliDomain> Cli<D> {
    ...
    /// [`Registry::register_persistent`](crate::Registry::register_persistent) (same bounds as
    /// [`Cli::register`], plus `H::World: Serialize`). A scenario profile's `export_world` key only
    /// works against a harness registered this way; against a plain [`Cli::register`]-ed harness it
    ...
    /// [`RuntimeFlavor::CurrentThread`](tokio::runtime::RuntimeFlavor::CurrentThread) — the erased
    /// registry layer is `!Send` by design, so a caller running this on a multi-thread runtime is a
    /// programming error, not a recoverable one. (2) installs a `tracing-subscriber` fmt layer
    /// honoring `RUST_LOG` (default `info`); double-init (e.g. across repeated test calls) is
    /// tolerated via `try_init()`. (3) loads `.env` via [`dotenvy::from_path`] if [`Cli::env_file`]
    /// is set; a missing file is logged at `debug` and is not fatal (a run may set secrets another
    /// way). (4) parses argv (naming the command [`CliDomain::BIN_NAME`]/[`CliDomain::ABOUT`] at
    /// parse time) and dispatches.
    pub async fn main(self) -> std::process::ExitCode {
        assert_eq!(
            tokio::runtime::Handle::current().runtime_flavor(),
            tokio::runtime::RuntimeFlavor::CurrentThread,
            "the CLI must run on a #[tokio::main(flavor = \"current_thread\")] runtime: \
        ...
        // Two-step parse (build the `clap::Command`, name it, then match + deserialize) so the
        // domain's `BIN_NAME`/`ABOUT` are stamped at parse time rather than baked into a `#[command]`
        // attribute on `CliArgs` (which cannot see the domain type parameter).
    ...
    /// `run <config> [--profile ...] [--suite ...] ...`. Loads the config, selects profiles (spec
    /// section 8's selection rules), runs each in turn, and returns the combined exit code.
    async fn dispatch_run(&self, args: RunArgs<D::Args>) -> u8 {
        let cfg = match load_config::<D::Ext>(&args.config) {
            Ok(c) => c,
            Err(msg) => {
                tracing::error!("{msg}");
                return 3;
    ...
    /// `replay <artifact>` (spec section 10): sugar for `run <artifact> --profile replay`. Reuses
    /// [`Cli::dispatch_run`] verbatim — an artifact is a valid config file with exactly one
    /// `[profile.replay]` scenario profile holding the (possibly shrunk) failing history, so this
    /// needs no bespoke loading, registry, or exit-code logic of its own. The domain flags given to
    /// `replay` are threaded through unchanged. Exit code `0` means the artifact's failure no longer
    /// reproduces (the bug was fixed); non-zero means it still does.
    async fn dispatch_replay(&self, args: ReplayArgs<D::Args>) -> u8 {
    ...
    /// The testable body of [`Cli::dispatch_run`]: everything after the config is already loaded, so
    /// tests can build a [`RunConfig`] in memory (no disk I/O).
    async fn run_with_config(&self, cfg: &RunConfig<D::Ext>, args: &RunArgs<D::Args>) -> u8 {
        ...
        let (phases, stop_on_failure) = match select_phases(cfg, args, &env, D::ENV_PREFIX) {
            Ok(v) => v,
            Err(msg) => {
                tracing::error!("{msg}");
                return 3;
        ...
        let mut opts = build_run_options(args, &env, D::ENV_PREFIX);

        // Cooperative ctrl-c for an endurance run: the driver polls `opts.stop` at the top of its
        // loop only (see the endurance runner), never around an in-flight `apply`, so a wallet lock
        // or in-flight broadcast is never severed. First ctrl-c asks the run to stop after its
        // current operation; a second forces a hard exit. The spawned task captures only the `Send`
        // `Arc<AtomicBool>`, so it is spawnable on this CLI's required
        // `#[tokio::main(flavor = "current_thread")]` runtime despite the rest of the registry being
        // `!Send`.
        ...
        let profiles_len = phases.len();
        let config_path = args.config.to_string_lossy();
        let code = run_selected::<D>(
            &self.registry,
            cfg,
            phases,
            opts,
        ...
        .await;
        tracing::info!(exit_code = code, profiles = profiles_len, "run summary");
        code
    }

    /// `validate <config>`. Loads the config, prints its loader warnings, and type-checks every
    /// profile against the registered harness — never touching a chain.
    ...
    fn validate_with_config(&self, cfg: &RunConfig<D::Ext>) -> u8 {
        for w in &cfg.warnings {
            tracing::warn!("{w}");
        }
    ...
    /// `list <config>`. Prints registered harness names, the config's profiles (name + mode), suites
    /// (name + members), and the `[harness]` in use.
    ...
    fn list_with_config(&self, cfg: &RunConfig<D::Ext>) -> u8 {
        tracing::info!(
...
/// Prints a clap parse error and maps it to an [`ExitCode`](std::process::ExitCode): clap's own
/// `exit_code()` is `0` for `--help`/`--version`, non-zero otherwise; a parse failure is always a
/// usage error (exit code `3`), never a run outcome.
fn exit_for_clap_error(e: clap::Error) -> std::process::ExitCode {
    let _ = e.print();
    std::process::ExitCode::from(if e.exit_code() == 0 { 0 } else { 3 })
}

/// The mode label a profile variant carries (used by `list`).
fn mode_label(profile: &Profile) -> &'static str {
    match profile {
        Profile::Fuzz(_) => "fuzz",
        Profile::Invariant(_) => "invariant",
        Profile::Endurance(_) => "endurance",
        Profile::Scenario(_) => "scenario",
    }
}

/// Loads and parses `path` into a [`RunConfig`], resolving `${VAR}` interpolation against the
/// process environment (after `.env` has already been folded in by [`Cli::main`]).
fn load_config<X: ConfigExt>(path: &Path) -> Result<RunConfig<X>, String> {
    harness_config::load::<X>(path, &std_env_lookup).map_err(|e| e.to_string())
}

/// `vars` closure shared by config loading and `{PREFIX}_*`/`PROPTEST_*` env folding: reads the real
/// process environment. A plain fn pointer (not a closure) so every call site can pass
...
/// Top-level argv, parsed by [`Cli::main`]. Deliberately not named `Cli` (that name is the public
/// builder in this module); kept private since only [`Cli::main`] parses it. Generic over the
/// domain flag bundle `A`, flattened into the `run`/`replay` subcommands. The command `name`/`about`
/// are set at parse time from [`CliDomain`], not via a `#[command]` attribute.
...
struct ReplayArgs<A: clap::Args> {
    ...
    artifact: PathBuf,
    /// Domain-specific flags, flattened in.
    #[command(flatten)]
    domain: A,
}

/// `run <config> [--profile NAME]... [--suite NAME] ...` (spec section 8).
#[derive(Debug, Clone, Default, clap::Args)]
struct RunArgs<A: clap::Args> {
    /// Path to the config file.
    config: PathBuf,
    /// Run this profile; repeatable. Mutually exclusive in practice with `--suite` (suite wins if
    /// both are given — see [`select_phases`]).
    #[arg(long = "profile")]
    profile: Vec<String>,
    ...
    /// Overrides the JSON report output path (spec section 9). The envelope is written once, after
    /// every selected profile has run, by [`run_selected`].
    ...
    #[arg(long = "no-shrink")]
    no_shrink: bool,
    /// Domain-specific flags (e.g. cross-vm's `--target`/`--target-chain`), flattened in.
    #[command(flatten)]
    domain: A,
}

// ---------------------------------------------------------------------------------------------
// Precedence folding (spec section 8): CLI flag > {PREFIX}_* env > profile/[defaults] (already
// folded by resolve_profile) > built-in default.
// ---------------------------------------------------------------------------------------------

/// Folds `args` and `env`-sourced `{prefix}_*`/`PROPTEST_CASES` overrides into a [`RunOptions`], CLI
/// winning over env. `--stats` is a bool flag: `Some(true)` when present so the profile's own
/// `stats` key stands when it is not. `prefix` is the domain's [`CliDomain::ENV_PREFIX`] (e.g.
/// `"HARNESS"` or `"CROSS_VM"`), parameterized so this pure fn stays unit-testable.
...
pub fn build_run_options<A: clap::Args>(
    args: &RunArgs<A>,
    ...
    let seed = args
        .seed
        .or_else(|| env(&format!("{prefix}_SEED")).and_then(|s| s.parse().ok()));
    let cases = args.cases.or_else(|| {
        env(&format!("{prefix}_CASES"))
            .or_else(|| env("PROPTEST_CASES"))
            .and_then(|s| s.parse().ok())
    });
    let ops = args
        .ops
        .or_else(|| env(&format!("{prefix}_OPS")).and_then(|s| s.parse().ok()));
    ...
    RunOptions {
        ...
        // Never folded here: `run_with_config` wires this to the ctrl-c signal task, which needs a
        // live `Arc` to flip, not anything derivable from `args`/`env` alone. Kept `None` in this
...
pub struct PhasePlan {
    ...
    world: WorldSource,
    /// Optional per-phase `params` table for the harness's registered world patch fn; `None` for a
    /// legacy path.
...
///
/// Order: `--suite NAME` (its own `phases` + `stop_on_failure`) beats one-or-more `--profile NAME`
/// (run in order, `stop_on_failure = false`) beats `{prefix}_PROFILE` (single profile) beats
/// "exactly one profile exists in the config" (auto-select). Otherwise: a usage error listing the
/// available names. An unknown `--suite`/`--profile`/`{prefix}_PROFILE` name is also a usage error
...
// unit-testable building block, so it is intentionally more visible than its argument.
#[allow(private_interfaces)]
pub fn select_phases<X: ConfigExt, A: clap::Args>(
    cfg: &RunConfig<X>,
    args: &RunArgs<A>,
    env: &dyn Fn(&str) -> Option<String>,
    prefix: &str,
    ...
    if let Some(suite_name) = &args.suite {
        ...
        // `Suite.phases` is the source of truth after loading (legacy `profiles` is normalized into
        // it, as dependency-free fresh phases). Dependency gating and world inheritance are honored
        // by `run_selected`; here we just carry each phase's shape through.
        let phases = suite
            .phases
            .iter()
            .map(|p| PhasePlan {
    ...
    if let Some(env_profile) = env(&format!("{prefix}_PROFILE")) {
...
/// Formats an "unknown `{kind}` `{name}`" usage-error message listing the available names, sorted
/// for a stable, testable message.
...
/// failed with `Infra`.
pub fn exit_code_for(report: &ErasedReport) -> u8 {
...
/// `export_world` against a harness that cannot export lands in `Invalid`, since `validate` is meant
/// to catch exactly this offline. `Setup` (the config-driven setup fn failed: deploy/RPC/model
/// desync), `Serialize` (the failure history could not be turned into JSON), and `Export` (a
/// `register_persistent` harness's `export_world` write itself failed: bad directory, permissions,
/// disk full) are not usage errors — nothing about the invocation was wrong — but neither are they a
/// discovered SUT bug, so all three map to `2` (infra-only failure), the same bucket a
/// `FailureKind::Infra` report gets.
pub fn exit_code_for_run_error(err: &RunError) -> u8 {
    match err {
...
/// The severity ranking [`combine`] uses: usage/config error (exit code `3`) dominates everything;
/// among run outcomes, a discovered bug/invariant violation (`1`) is worse than an infra-only
/// failure (`2`), which is worse than a clean pass (`0`). This is spec section 8's ordering ("a
/// suite reports the worst code") made precise: exit-code numbers are not monotonic with severity
/// (`1` is worse than `2`), so comparing them directly would be wrong; this function is the one
...
/// Runs every phase in `phases` against `cfg`'s harness, in declaration order, combining exit codes
/// per [`combine`], with dependency gating and world handoff (spec: pipeline suites). The domain
/// seam is threaded through here: `D::build_setup` builds each run's setup value, `D::artifact_sections`
/// adds the domain's top-level sections to a replay artifact, and `D::overrides_json` records the
/// domain's flags in the JSON report's `invocation.overrides`.
///
/// A phase whose any `needs` entry did not [`PhaseOutcome::Passed`] is skipped: it contributes
/// nothing to the exit code (a skip is not a failure). A phase that runs sets its `world_source` from
/// the plan and stashes its ending world (`stash_world`) exactly when a later phase inherits from it,
/// so the registry's session slot hands the `(Ctx, World)` forward. When `stop_on_failure` is set,
/// stops after the first phase whose code is non-zero. The ctrl-c stop flag (`opts.stop`) is checked
/// between phases only, never mid-run.
///
/// Accumulates every ran phase's [`ErasedReport`] into one `Vec`, and — if a JSON report path is set,
/// either `opts.json_report` (the CLI `--json-report` flag, checked first) or the first resolved
/// profile's own `json_report` key — writes the whole invocation's envelope exactly once at the end
/// via [`write_json_report`], never per-phase (spec section 9). `config_path` is the config file path
/// exactly as the user passed it; the envelope records every selected phase's profile name.
async fn run_selected<D: CliDomain>(
    ...
    for (i, plan) in phases.iter().enumerate() {
        ...
        resolved.stash_world = phases.iter().skip(i + 1).any(|later| {
            later.world == WorldSource::Inherit && later.needs.first() == Some(&plan.profile)
        });
        ...
        // Build the per-run setup factory the domain owns, then drive the profile through the
        // registry with it. The `u64` is the concrete per-case seed the registry supplies.
        let make_setup = |seed: u64| D::build_setup(cfg, &resolved, domain_args, seed);
        let outcome = registry
            .run(&cfg.harness.name, &resolved, &opts, &make_setup)
            .await;
        ...
        match outcome {
            Ok(report) => {
                ...
                // endurance): a scenario is already a concrete, checked-in sequence, and a passing
                // run has no failure to reproduce.
                if report.failure.is_some() && report.mode != "scenario" {
                    let sections = D::artifact_sections(cfg, &resolved, domain_args);
                    let written = write_replay_artifact(
                    ...
                    match written {
                        Ok(path) => {
                            tracing::info!(
                                "wrote replay artifact: {}; reproduce with: replay {}",
                                path.display(),
                                path.display()
                            );
                            ...
                            if plan.world == WorldSource::Inherit {
                                tracing::warn!(
                        ...
                        // failure; a write error here (bad directory, permissions, disk full) is a
                        // secondary concern, logged but never overriding `this_code`.
                        Err(e) => tracing::warn!(
                            profile = %resolved.name,
                            error = %e,
    ...
    if let Some(path) = &json_report_path {
        ...
        match write_json_report(Path::new(path), config_path, &names, &reports, overrides) {
            ...
            Err(e) => {
                // An IO failure here (bad directory, permissions, disk full) is a property of the
                // invocation's own `--json-report`/`json_report` argument, not of anything the run
                // discovered about the system under test: it belongs in the same usage/config bucket
                // (exit 3) as an unresolvable profile name or a malformed config, not the infra
                // bucket (2) reserved for chain/RPC/deploy failures during a run. `combine` folds it
                // in, so it dominates a clean pass but never silently downgrades a worse code a
                // profile already reported.
...
/// Builds the `invocation.overrides` object for a [`write_json_report`] call: every CLI-set scalar on
...
mod tests {
    ...
    fn failing_report(kind: FailureKind) -> ErasedReport {
        ErasedReport {
            failure: Some(crate::erased::ErasedFailure {
                step: 1,
                kind,
                op_debug: None,
    ...
    fn validate_subcommand_parses() {
        let args = CliArgs::<NoArgs>::try_parse_from(["harness", "validate", "f.toml"]).unwrap();
    ...
    fn replay_subcommand_parses() {
        let args =
            CliArgs::<NoArgs>::try_parse_from(["harness", "replay", "f.replay.toml"]).unwrap();
    ...
    #[test]
    fn parses_run_with_two_profiles_and_seed() {
        let args = CliArgs::<NoArgs>::try_parse_from([
            "harness",
            "run",
            "f.toml",
            "--profile",
    ...
    fn run_args_with_seed(seed: Option<u64>) -> RunArgs<NoArgs> {
        RunArgs {
            seed,
            ..Default::default()
        }
    ...
    fn cli_seed_wins_over_env() {
        ...
        let opts = build_run_options(&args, &env, "HARNESS");
    ...
    #[test]
    fn cases_folds_from_harness_cases_then_proptest_cases() {
        let args = RunArgs::<NoArgs>::default();
        let env = |k: &str| {
            if k == "PROPTEST_CASES" {
                Some("42".to_string())
            } else {
    ...
    fn stats_flag_present_is_some_true_absent_is_none() {
        let mut args = RunArgs::<NoArgs>::default();
        let env = |_: &str| None;
        assert_eq!(build_run_options(&args, &env, "HARNESS").stats, None);
        args.stats = true;
        assert_eq!(build_run_options(&args, &env, "HARNESS").stats, Some(true));
    }
...
"#;
    ...
    fn single_profile_config_auto_selects() {
        ...
        let args = RunArgs::<NoArgs>::default();
        let env = |_: &str| None;
        let (phases, stop) = select_phases(&cfg, &args, &env, "HARNESS").unwrap();
    ...
    fn multi_profile_with_no_selector_is_a_usage_error_listing_names() {
        ...
        let args = RunArgs::<NoArgs>::default();
        let env = |_: &str| None;
        let err = select_phases(&cfg, &args, &env, "HARNESS").unwrap_err();
    ...
    fn explicit_profile_flags_win_and_run_in_order() {
        ...
        assert!(phases
            .iter()
            .all(|p| p.needs.is_empty() && p.world == WorldSource::Fresh));
        assert!(!stop);
    }
    ...
    #[test]
    fn harness_profile_env_selects_when_no_flag_given() {
        let cfg = load(MULTI_PROFILE);
        let args = RunArgs::<NoArgs>::default();
        let env = |k: &str| {
            if k == "HARNESS_PROFILE" {
                Some("deep".to_string())
            } else {
                None
            }
...
"#,
        ...
        // Steps aren't observable through the exit code alone, but a bad override would panic or
        // error inside `run_with`, so a clean 0 here demonstrates the override was accepted and
        // threaded through `RunOptions` end to end via the CLI dispatch path.
        assert_eq!(cli.run_with_config(&cfg, &args).await, 0);
    }
    ...
    fn overrides_json_never_includes_json_report_or_artifacts_dir() {
        // Both are file paths, not "run-shape" overrides; asserting their absence documents that
        // this function's field list is deliberately narrow, not merely incomplete.
        let opts = RunOptions {
    ...
    /// A fresh temp file path under `<CARGO_MANIFEST_DIR>/tests_result/`, unique per test invocation
    /// (process id + a monotonic counter), so parallel test runs never collide.
    fn temp_json_path(label: &str) -> PathBuf {
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests_result")
            .join(format!(
                "harness-cli-json-report-{}-{}-{label}.json",
                std::process::id(),
                n
            ))
    }
...
"#,
        ...
        assert_eq!(value["invocation"]["config"], "vault.toml");
        assert_eq!(
        ...
        // One entry per profile in the invocation: the envelope is written once, not per profile, so
        // both selected profiles land in the same file's `profiles` array.
...
"#,
    ...
    // -----------------------------------------------------------------------------------------
    // Replay artifacts (spec section 10): written on a generative failure by `run_selected`, never
    // for a pass or a scenario run; `replay <artifact>` (`dispatch_replay`) is sugar for `run
    // <artifact> --profile replay` and reproduces the same failure.
    // -----------------------------------------------------------------------------------------
    ...
    fn temp_artifacts_dir(label: &str) -> PathBuf {
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests_result")
            .join(format!(
                "harness-cli-replay-artifact-{}-{}-{label}",
                std::process::id(),
                n
            ))
    }
...
"#,
        ...
        // `replay <artifact>` must reproduce the exact same failure (still-broken SUT means it still
        // fails; exit code 1, same as the original run).
        let code = cli
    ...
    /// test invocation, so parallel runs never collide and nothing leaks into a source-tree
    ...
    fn temp_export_path(label: &str) -> PathBuf {
        ...
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests_result")
            .join(format!(
                "harness-cli-pipeline-export-{}-{}-{label}.json",
                std::process::id(),
                n
            ))
    }
