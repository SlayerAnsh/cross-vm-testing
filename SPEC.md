...
* The RPC backend (`TronRpcProvider`) drives a live java-tron node over the TronGrid HTTP REST API (`/wallet/*` endpoints, via `reqwest`). Real reads (`balance`, `block_height`, `static_call`) and signed writes (`deploy_create` and `call` build the unsigned transaction at the node, sign its `txID` locally, then broadcast) are in place; only `set_balance` returns `Unimplemented`, since a live chain cannot mint. The per-tx event-log fetch (`GET /v1/transactions/{txid}/events`) and range search (`eth_getLogs`, TronGrid `/v1/contracts/{addr}/events`) remain a later enhancement for richer log retrieval, so `call` returns the tx hash with empty logs for now.
