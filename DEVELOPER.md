...
`.github/workflows/ci.yml` runs on push to `main`, on pull requests, and on manual dispatch. It is a single job so the contract artifacts are built once and reused across every test step (the cargo target directory is shared). The order is fail-fast: the cheap artifact-free checks run first (`cargo fmt --check`, `cargo clippy --workspace -- -D warnings`, `cargo doc --workspace --no-deps`, `cargo build --workspace`), so a trivial failure never pays for the toolchain installs. Then it installs Foundry, Solana (Agave platform tools from Anza, since the legacy `release.solana.com` host is flaky), and Anchor (the prebuilt npm `@coral-xyz/anchor-cli`, `chmod +x`'d because npm ships it non-executable), builds the embedded artifacts (`make compile-solidity`, `make compile-solana`), and runs the tests in two steps: `cargo test --workspace --exclude cross-vm-cosmwasm`, then `cargo test -p cross-vm-cosmwasm --lib --test counter --test vault --test typed_handle`.

`cross-vm-cosmwasm` is split out because `tests/onchain.rs` embeds a compiled `counter.wasm` that only the Docker CosmWasm optimizer produces (not available in CI); it is a live `#[ignore]` test, and the cross-VM cosmwasm path is still exercised through `cw-multi-test`'s in-process `ContractWrapper`, which needs no wasm. The pinned Anchor and Solana versions may need bumping together if `anchor build` fails on a platform-tools mismatch.
