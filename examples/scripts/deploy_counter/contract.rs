...
use crate::{describe, describe_units};
...
impl Counter {
    ...
    async fn cw_increment(&self, wallet: &str) -> Result<AppResponse<()>, CrossVmError> {
        ...
        // The derive threads the gas limit through as a trailing argument, so a typed call is
        // no less explicit about cost than the raw `execute_contract` underneath it.
        use cw::ExecuteMsgFns;
        let chain = self.base.cosmwasm()?;
        let addr = self.base.cw_addr()?;

        // The forecast `CwGasLimit::Estimated` is about to resolve from, printed next to the
        // receipt `main` prints for the same op. The two are the same quantity (gas units), so
        // they are directly comparable; the limit sits above both, by `gas_adjustment`.
        let forecast = chain
            .estimate_execute_contract(
                &addr,
                cw::ExecuteMsg::Increment {},
                WalletLabel::wrap(wallet),
                &[],
            )
            .await?;
        println!(
            "[cosmwasm] increment forecast: {}",
            describe_units(forecast)
        );

        let raw = chain
            .contract_as::<cw::CounterContract>(addr)
