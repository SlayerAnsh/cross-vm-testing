//! Single-VM (CosmWasm) `Counter` adapter: re-exports the shared `Counter` wrapper + harness builder from
//! `cross-vm-common` and pins them to this crate's chain (the mock/rpc `OSMOSIS` preset under
//! `"osmosis"`).
//!
...
pub async fn counter_setup(_seed: u64) -> Result<(Ctx, CounterWorld), HarnessError> {
    setup_on(LABEL, |w| OSMOSIS.mock(w).into()).await
}

/// The config-driven counterpart of [`counter_setup`], registered with the `cross-vm` CLI. Falls
/// back to the mock (or rpc) `OSMOSIS` preset under `"osmosis"` when the TOML declares no `[[chain]]`.
pub fn counter_config_setup(req: SetupRequest) -> SetupFuture<'static, CounterWorld> {
    config_setup_with(req, LABEL, |target, w| match target {
        Target::Mock => OSMOSIS.mock(w).into(),
        Target::Rpc => OSMOSIS.rpc(w).into(),
    })
}
