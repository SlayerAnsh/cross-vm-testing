//! Single-VM (EVM) `Counter` harness, driven three ways from one place.
//!
//! [`Counter`] is a thin wrapper over the EVM counter contract (deploy, increment, read). The
...
//! honoring a `[[chain]]` declaration and falling back to the mock `ETHEREUM` preset otherwise.

use cross_vm_common::mocks::counter::evm as counter_evm;
...
/// A minimal EVM `Counter` handle: deploy, increment, read the count.
pub struct Counter {
    base: ContractBase,
}
...
impl Counter {
    ...
    pub async fn increment(&self, wallet: &str) -> Result<(), CrossVmError> {
        ...
        let calldata = Bytes::from(counter_evm::Counter::incrementCall {}.abi_encode());
    ...
    pub async fn count(&self) -> Result<u64, CrossVmError> {
        ...
        let out = chain
            .static_call(
                &addr,
                Bytes::from(counter_evm::Counter::countCall {}.abi_encode()),
            )
            .await?;
        let n =
            counter_evm::Counter::countCall::abi_decode_returns(&out).map_err(|e| {
                CrossVmError::Query {
                    kind: ChainKind::Evm,
                    reason: e.to_string(),
                }
...
pub async fn counter_setup(_seed: u64) -> Result<(Ctx, CounterWorld), HarnessError> {
    ...
    env.inject(LABEL, ETHEREUM.mock(wallets.clone()));
...
/// loaded TOML declares no `[[chain]]`, this injects the mock (or rpc) `ETHEREUM` preset under
/// `"eth"`, honoring `req.target`. Otherwise it builds the first declared chain via [`build_chain`]
    ...
    Box::pin(async move {
        ...
        let label = if let Some(spec) = req.chain_specs.first() {
            ...
            let chain: AnyChain = match req.target {
                Target::Mock => ETHEREUM.mock(wallets.clone()).into(),
                Target::Rpc => ETHEREUM.rpc(wallets.clone()).into(),
            };
